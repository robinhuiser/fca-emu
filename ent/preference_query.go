...
// Returns a *NotSingularError when exactly one Preference entity is not found.
...
// Returns a *NotSingularError when exactly one Preference ID is not found.
...
func (pq *PreferenceQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if ps := pq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector, preference.ValidColumn)
			}
		}
	}
	return _spec
...
func (pq *PreferenceQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	selector := builder.Select(t1.Columns(preference.Columns...)...).From(t1)
	if pq.sql != nil {
		selector = pq.sql
		selector.Select(selector.Columns(preference.Columns...)...)
	}
	for _, p := range pq.predicates {
		p(selector)
	}
	for _, p := range pq.order {
		p(selector, preference.ValidColumn)
	}
	if offset := pq.offset; offset != nil {
...
func (pgb *PreferenceGroupBy) sqlQuery() *sql.Selector {
	...
	for _, fn := range pgb.fns {
		columns = append(columns, fn(selector, preference.ValidColumn))
	}
...
func (ps *PreferenceSelect) sqlQuery() sql.Querier {
	selector := ps.sql
