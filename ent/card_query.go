...
func (cq *CardQuery) QueryNetwork() *CardNetworkQuery {
	query := &CardNetworkQuery{config: cq.config}
...
func (cq *CardQuery) sqlAll(ctx context.Context) ([]*Card, error) {
	...
	if query := cq.withNetwork; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Card)
		for i := range nodes {
			fk := nodes[i].card_network
			if fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(cardnetwork.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "card_network" returned %v`, n.ID)
			}
			for i := range nodes {
