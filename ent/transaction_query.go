...
func (tq *TransactionQuery) QueryImages() *BinaryItemQuery {
	query := &BinaryItemQuery{config: tq.config}
...
func (tq *TransactionQuery) sqlAll(ctx context.Context) ([]*Transaction, error) {
	...
	if query := tq.withImages; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Images = []*BinaryItem{}
		}
		query.withFKs = true
		query.Where(predicate.BinaryItem(func(s *sql.Selector) {
			s.Where(sql.InValues(transaction.ImagesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.transaction_images
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "transaction_images" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
